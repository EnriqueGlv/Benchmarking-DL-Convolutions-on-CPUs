...
#ifndef CPU_LIP6_CONVOLUTION_IM2ROW_HPP
#define CPU_LIP6_CONVOLUTION_IM2ROW_HPP
...
struct lip6_convolution_im2row_fwd_t : public primitive_t {
    struct pd_t : public cpu_convolution_fwd_pd_t {
        using cpu_convolution_fwd_pd_t::cpu_convolution_fwd_pd_t;

        DECLARE_COMMON_PD_T("lip6:im2row", lip6_convolution_im2row_fwd_t);
        ...
        status_t init(engine_t *engine) {
            ...
            bool ok = is_fwd()
                    ...
                    && !getenv_int("LIP6_DISABLE_IM2ROW", 0)
                    ;
    ...
    protected:
        ...
        status_t alloc_mem(float* &im2row){
            const memory_desc_wrapper src_d(src_md());
            const memory_desc_wrapper weights_d(weights_md(0));

            const auto com_size = IC() * KH() * KW();
            const auto im2row_size = MB() * OH() * OW() * com_size;
            im2row = new float[im2row_size];
            
            return status::success;
        }
    };

    lip6_convolution_im2row_fwd_t(const pd_t *apd) : primitive_t(apd) {}
